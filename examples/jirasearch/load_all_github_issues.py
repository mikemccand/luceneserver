...
def create_db(db):
    ...
    c.execute('CREATE TABLE issues (key TEXT, pickle BLOB)')
    ...
    c.execute('CREATE UNIQUE INDEX key_index ON issues(key)')
    db.commit()
    return db
...
def main(g, db):
    ...
    with closing(db):
        ...
        for count, issue in enumerate(issues):
            ...
            print(f'{count}: number={number} {issue} created={created_at} updated={updated_at} closed_at={closed_at}')
            ...
            c.execute('INSERT OR REPLACE INTO issues (key, pickle) VALUES (?, ?)',
                      (str(number), pickle.dumps((issue, labels, comments, events, reactions, timeline))))
